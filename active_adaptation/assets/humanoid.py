...
GR3_DESCRIPTION_DIR = os.path.join(BASE_DIR, "gr3_description")
...
GR3_CFG = ArticulationCfg(
    spawn=sim_utils.UrdfFileCfg(
        fix_base=False,
        replace_cylinders_with_capsules=True,
        asset_path=f"{GR3_DESCRIPTION_DIR}/gr3_fourier_hand_6dof.urdf",
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            retain_accelerations=False,
...
GR3_ACTION_SCALE = {}
for a in GR3_CFG.actuators.values():
    e = a.effort_limit_sim
    s = a.stiffness
    names = a.joint_names_expr
    if not isinstance(e, dict):
    ...
    for n in names:
        if n in e and n in s and s[n]:
            GR3_ACTION_SCALE[n] = 0.25 * e[n] / s[n]